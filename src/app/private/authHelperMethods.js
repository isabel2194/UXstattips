...
export default class AuthHelperMethods {
  ...
  constructor(domain) {
    ...
    this.domain = domain || "https://uxserverstattips.herokuapp.com"; // API server domain
  }
  login = (email, passwd) => {
    // Get a token from api server using the fetch api
    //return this.fetch("http://localhost:3001/client/login", {
    return this.fetch("https://uxserverstattips.herokuapp.com/client/login", {
      method: "POST",
      body: JSON.stringify({
        email: email,
        password: passwd
  ...
  setWebPage = email => {
    localStorage.setItem("website", email);
  };
